...
class node
{ //node datatype consists of int and a pointer of another node.
	int data;
	node *address;
...
{ //function returns a pointer of node type
	node *temp;
...
void node::insertNode()
{
	...
	cin >> n;
	while (n != -1)
	{						 //will iterate through this loop until user enters -1
		...
		{					  //if list not empty
			...
			{								  //will iterate while we reach last node
...
void node::printList()
{
	...
	{
		while (traverse->address != NULL)
		{ //start a loop till node address is not null
			cout << traverse->data << " -> ";
...
void node::deleteNode()
{
	...
	else
	{
		...
		if (ptr->data == key)
		{						  //if key found at the very first node
		...
		else
		{
			...
			while (ptr->address != NULL && ptr->data != key)
			{ //iterate till the condition specified
				ptr1 = ptr;
				ptr = ptr->address;
			}
			...
			else if (ptr->address == NULL && ptr->data == key)
			{						  // key found at the last node
...
void node::sortList()
{
	...
	else
	{
		...
		{					 //t will traverse till the last node(init with header node)
			t1 = t->address; //t1 takes the address of the next node
			while (t1->address != NULL)
			{ //t1 will traverse the whole list
				if (t->data > t1->data)
				{ //previous data found larger than the subsequet data
					// temp;
			...
			if (t1->address == NULL && t->data > t1->data)
			{ //corner case for the last node handled
				temp = t1->data;
				t1->data = t->data;
				t->data = temp;
			}
...
void node::addElementBeg()
{
	...
	cout << endl
		 << "Your node successfuly inserted in the beginning" << endl;
}
...
int node::noNode(node *t)
{ // method to calc the no of nodes present after a specific node
	int count = 0;
...
void node::nRev()
{ // algorithm by brute force technique
	int n;
	node *t = start;
...
void node::numNode()
{ // method for finding the number of nodes in the list
	int count = 0;
	node *t = start;
