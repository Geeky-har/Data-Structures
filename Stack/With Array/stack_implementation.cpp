...
class ArrayStack{	// class for array stack
	int top;
	int size;
	int *array;
...
ArrayStack* ArrayStack:: CreateStack(){		// function for creating a stack of defined size
...
bool ArrayStack:: isFull(ArrayStack *stack){		// method to check if array is full?
...
bool ArrayStack:: isEmpty(ArrayStack *stack){	// method to check if array is empty?
...
void ArrayStack:: push(ArrayStack *stack){		// method for push operation
	int item;
	
	if(!isFull(stack)){	// execute if stack is not full
...
void ArrayStack:: pop(ArrayStack *stack){		// method for pop operation
	int temp;
	
	if(!isEmpty(stack)){	// execute if stack not empty
...
void  ArrayStack:: printStack(ArrayStack *stack){	// for printing the elements in the stack
	int i;
	if(isEmpty(stack)){
		cout<<"The stack is empty, please perform push operation"<<endl;
	}
	
	else{	// will execute if the stack is not empty
...
int main(){
	...
	do{
		...
		switch(option){
			case 1: stack = obj.CreateStack();		// pointer stores the address of new class
					break;
						
			case 2: obj.push(stack);		// push operation with class pointer as args
					break;
						
			case 3: obj.pop(stack);			// pop operation with class pointer as args
					break;
						
			case 4: obj.printStack(stack);  // print operation with class pointer as args
					break;
		}
